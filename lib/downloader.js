...
function writeDescription(desc, dir) {
  writeTextFile(desc, descriptionName, dir);
}

/**
 * Synchronously writes the URL file.
 *
 * @param {String} url URL to the Soundcloud page
 * @param {String} dir Base directory
 */
function writeURL(url, dir) {
  writeTextFile(url, urlName, dir);
}
 ...
 * @param {String} dir Base directory
 */
function writeTextFile(content, name, dir) {
  content = content.trim();
  if (content === '') {
    console.log('scscrape.js: Not writing ' + name + ' since the content would be empty.');
    return;
  }
